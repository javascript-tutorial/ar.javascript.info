...
فمثلاً لو بحثنا الآن عن خاصية ما في كائن `rabbit` ولم تكُ موجودة، ستأخذها لغة جافا سكريبت تلقائيًا من كائن `animal`.
...
1. لا يمكن أن تكون سلسلة الوراثة النموذجية دائرية (على شكل حلقة). ما إن تُسند `__proto__` بطريقة دائرية فسترمي لغة جافا سكريبت خطأً.
2. يمكن أن تكون قيمة `__proto__` إمّا كائنًا أو `null`، وتتجاهل لغة جافا سكريبت الأنواع الأخرى.
=======
