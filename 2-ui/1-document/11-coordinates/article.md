...
````عنوان تحذيري="بالنسبة إلى إحداثيات خارج النافذة ، يُرجع` elementFromPoint` `القيمة الخالية`لا تعمل الطريقة`document.elementFromPoint (x، y)` إلا إذا كانت "(x، y)` داخل المنطقة المرئية.
